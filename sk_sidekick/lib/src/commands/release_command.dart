...
class ReleaseCommand extends Command {
  ...
  Future<void> run() async {
    ...
    if (package == null) {
      ...
      print(
        'Do you want to release a new version for '
  ...
  Future<void> _releasePackage(DartPackage package) async {
    ...
    while (_gitRepoHasChangesIn(package.root)) {
      ask(
        red(
          'package:${package.name} contains local changes. '
...
${changelog.readAsStringSync().replaceFirst('# Changelog', '').trimLeft()}''');
    ...
    final newChangelogAndVersionBranch = _getCurrentBranch(
      SidekickContext.repository!,
    );
  ...
  String _prepareNextReleaseChangelog(DartPackage package) {
    ...
    final packageChanges = _getChanges(
      from: currentPackageVersionTag,
      paths: [package.root.path],
    );
...
''');
  ...
  List<String>? _getSidekickCoreChanges(DartPackage package) {
    final currentSidekickCoreVersion = _getMinSidekickCoreVersion(
      package.pubspec.readAsStringSync(),
    );
    if (currentSidekickCoreVersion != null) {
      ...
      final oldPubspecContent =
          'git show $currentSidekickCoreVersionTag:${package.name}/pubspec.yaml'
              ...
              .lines
              .join('\n');
      final oldSidekickCoreVersion = _getMinSidekickCoreVersion(
        oldPubspecContent,
      );
